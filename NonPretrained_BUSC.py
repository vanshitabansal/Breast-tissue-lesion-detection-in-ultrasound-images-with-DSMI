...
class lesiondataset(torch.utils.data.Dataset):
    def __init__(self,root,folder='dataset',transforms=None):
        ...
        self.root=root
        self.folder=folder
        box_data=pd.read_csv("BUSC_Dataset/train.csv")
    ...
    def __getitem__(self,idx):
        ...
        if df.shape[0]!=0:
          ...
          if self.folder=="BUSC_Dataset/Train/benign/":
              labels=np.ones(len(boxes))
          elif self.folder=="BUSC_Dataset/Train/malignant/":
...
class lesiondataset2(torch.utils.data.Dataset):
    ...
    def __getitem__(self,idx):
        ...
        if df.shape[0]!=0:
          boxes=df[[0,1,2,3]].astype(float).values
          if self.folder=="BUSC_Dataset/Test/benign/":
              labels=np.ones(len(boxes))
          elif self.folder=="BUSC_Dataset/Test/malignant/":
              labels=np.full(len(boxes),2)
          else:    
              labels=np.full(len(boxes),0)
        else:
...
dataset_benign_train=lesiondataset(root,'BUSC_Dataset/Train/benign/',transforms=train_transforms)
dataset_benign_test=lesiondataset2(root,'BUSC_Dataset/Test/benign/',transforms=train_transforms)
image,target = dataset_benign_train.__getitem__(1)
print(image.shape)
plt.imshow(image.permute(1, 2, 0))
...
dataset_malignant_train=lesiondataset(root,'BUSC_Dataset/Train/malignant/',transforms=train_transforms)
dataset_malignant_test=lesiondataset2(root,'BUSC_Dataset/Test/malignant/',transforms=train_transforms)


#dataset_normal=lesiondataset(root,'BUSC_Dataset/normal/',transforms=train_transforms)
...
# checkpoint_path = 'model_nonpretrained_busc_checkpoint.pth'
...
#           if itr % 25 == 0:
#               print("\n Iteration #{} loss: {} \n".format(itr, loss_dict))

#           itr += 1
#           logging.basicConfig(filename='nonpretrained_busc.log', level=logging.DEBUG)
...
#     print('Epoch train loss is {:.4f}'.format(epoch_train_loss))
#     print('Epoch val loss is {:.4f}'.format(epoch_val_loss))
...
# plt.savefig('train_loss_epoch_nonpretrained_busc.png')


model.load_state_dict(torch.load('nonpretrained_busi.pth',map_location=torch.device('cpu')))
