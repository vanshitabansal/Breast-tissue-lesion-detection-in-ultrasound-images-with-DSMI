...
for epoch in tqdm(range(startepoch,num_epochs)):
    ...
    for images,targets in tqdm(data_loader_train):
        ...
        if box[0] >= box[2] or box[1] >= box[3]:
          ...
          itr += 1
          logging.basicConfig(filename='Nonpretrained_busi.log', level=logging.DEBUG)
    ...
    if epoch_val_loss < best_val_loss:
        ...
        epochs_no_improve = 0
        torch.save(model.state_dict(),'nonpretrained_busi.pth')
        checkpoint = {
             'epoch': epoch,
...
plt.savefig('train_loss_epoch_nonpretrained.png')
